...
void parseOptions(int argc, const char *argv[]) {
  ...
  LedState changes[] = {NoChange, NoChange, NoChange,
...
void explainUsage(void) {
  printf("Usage:\tsetleds [-name wildcard] [[+|-|^][ num | caps | scroll "
         "| compose | kana ]]\n"
...
void setKeyboard(IOHIDDeviceRef device, CFDictionaryRef keyboardDictionary,
                 LedState changes[]) {
  ...
  const char *deviceName =
      CFStringGetCStringPtr(deviceNameRef, kCFStringEncodingUTF8);

  if (nameMatch && fnmatch(nameMatch, deviceName, 0) != 0)
    return;

  CFArrayRef elements = IOHIDDeviceCopyMatchingElements(
      device, keyboardDictionary, kIOHIDOptionsTypeNone);
  bool missingState = false;

  if (elements) {
    long elementCount = CFArrayGetCount(elements);
    bool printedName = false;
    for (CFIndex elementIndex = 0; elementIndex < elementCount;
         elementIndex++) {
      IOHIDElementRef element =
          (IOHIDElementRef)CFArrayGetValueAtIndex(elements, elementIndex);
      ...
      if (element && kHIDPage_LEDs == IOHIDElementGetUsagePage(element)) {
        ...
        if (currentValue == 0x00) {
          ...
          if (changes[led] != NoChange && changes[led] != current) {
            ...
            IOHIDValueRef newValue = IOHIDValueCreateWithIntegerValue(
                kCFAllocatorDefault, element, 0, newState);
            if (newValue) {
              IOReturn changeResult =
  ...
  if (missingState) {
    fprintf(stderr,
            "\nSome state could not be determined. Please try running as "
            "root/sudo.\n");
  }
}
...
void setAllKeyboards(LedState changes[]) {
  ...
  if (!keyboard) {
    fprintf(stderr,
            "Failed to get dictionary usage page for kHIDUsage_GD_Keyboard.\n");
    return;
  }
